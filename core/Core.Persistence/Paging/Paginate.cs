...
public class Paginate<T> : IPaginate<T>
{
    ...
    public Paginate(IEnumerable<T> source, int index, int size, int from)
    {
        ...
        Count = source is IQueryable<T> queryable
            ? queryable.Count()
            : enumerable.Count();
...
public class Paginate<TSource, TResult> : IPaginate<TResult>
{
        ...
        int index, int size, int from)
    {
        ...
        Size = size;
        From = from;
        Count = source is IQueryable<TSource> queryable ? queryable.Count() : enumerable.Count();
        Pages = (int)Math.Ceiling(Count / (double)Size);

        var items = source.Skip((Index - From) * Size).Take(Size).ToArray();
